  ...
  $.get(url, function (data) {
    ...
    if (membership != "dyn-empty") {
      ...
      for (var i in data) {
        if (i.indexOf("tag_" + j + "_id") >= 0) {
          ...
          <a data-tag-text="' +
              data["tag_" + j + "_name"] +
              '" class="btn btn-primary btn-sm mb-1 tagremove" id="tag_id' +
              data[i] +
              '" data-tag-id=' +
              data[i] +
              ' href="#" style="color: #fff; text-align: center;" role="button">\
            <span>\
              ' +
              data["tag_" + j + "_name"] +
              '\
              <button type="button" class="myclose ml-2" data-tag-id=' +
              data[i] +
              '>\
    ...
    $("#btn-modal-post").click(function (event) {
      ...
      $("a[id^='tag_']").each(function () {
        console.log("POST data field: " + $(this).data("tag-text"));
        arr.push($(this).attr("data-tag-text"));
      });

      $("input[id='" + membership + "']").val(arr);
      console.log("GET input tags: " + $("input[id='tags']").val());
